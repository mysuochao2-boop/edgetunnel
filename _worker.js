			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (访问路径 === 'admin/init') {// 重置配置为默认值
						try {
						...
						} else if (访问路径 === 'admin/proxy-profiles') {
							return new Response(反代配置集管理页面(), { status: 200, headers: { 'Content-Type': 'text/html;charset=utf-8', 'Cache-Control': 'no-store' } });
							...
							if (访问路径 === 'admin/config.json') { // 保存config.json配置
								try {
									...
									if (config_JSON.反代?.配置集 && !newConfig.反代?.配置集) {
										if (!newConfig.反代) newConfig.反代 = {};
										newConfig.反代.当前配置 = config_JSON.反代.当前配置;
										newConfig.反代.配置集 = config_JSON.反代.配置集;
									}
									同步当前反代到配置集(newConfig);
									初始化反代配置集(newConfig);
							...
							} else if (访问路径 === 'admin/proxy-profiles.json') {
								try {
									const payload = await request.json();
									const result = await 处理反代配置集操作(env, config_JSON, payload);
									ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Save_Proxy_Profiles', config_JSON));
									return new Response(JSON.stringify(result, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
								} catch (error) {
									return new Response(JSON.stringify({ success: false, error: error.message }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
								}
							...
							return new Response(JSON.stringify(构建反代配置集响应(config_JSON), null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8', 'Cache-Control': 'no-store' } });
...
function 反代配置ID合法(id) {
	return typeof id === 'string' && /^[\w.-]{1,64}$/.test(id);
}

function 获取反代配置ID(input = {}) {
	return String(input.id || input.ID || input.key || input.name || input.名称 || '').trim();
}

function 复制SOCKS5配置(配置 = {}) {
	return {
		启用: 配置.启用 || null,
		全局: 配置.全局 === true || 配置.全局 === 'true',
		账号: 配置.账号 || '',
		白名单: Array.isArray(配置.白名单) ? 配置.白名单 : SOCKS5白名单,
	};
}

function 规范化反代配置项(input = {}, fallback = {}) {
	const _p = 特征码字典[0];
	const fallbackSocks = fallback.SOCKS5 || {};
	const inputSocks = input.SOCKS5 || input.socks5 || {};
	const 指定协议 = input.类型 !== undefined || input.type !== undefined || inputSocks.启用 !== undefined;
	const 协议 = String(指定协议 ? (input.类型 || input.type || inputSocks.启用 || '') : (fallbackSocks.启用 || '')).toLowerCase();
	const 支持协议 = ['socks5', 'http', 'https', 'turn', 'sstp'];
	const profile = {
		名称: String(input.名称 || input.name || fallback.名称 || '默认配置').trim() || '默认配置',
		[_p]: String(input[_p] ?? input.PROXYIP ?? input.proxyip ?? fallback[_p] ?? 'auto').trim() || 'auto',
		SOCKS5: 复制SOCKS5配置({ ...fallbackSocks, ...inputSocks }),
	};

	if (协议 && 支持协议.includes(协议)) {
		profile.SOCKS5.启用 = 协议;
		const 全局值 = input.全局 ?? input.global ?? inputSocks.全局 ?? fallbackSocks.全局 ?? false;
		profile.SOCKS5.全局 = 全局值 === true || 全局值 === 'true';
		profile.SOCKS5.账号 = String(input.账号 || input.account || inputSocks.账号 || fallbackSocks.账号 || '').trim();
		if (Array.isArray(input.白名单)) profile.SOCKS5.白名单 = input.白名单;
		if (Array.isArray(inputSocks.白名单)) profile.SOCKS5.白名单 = inputSocks.白名单;
	} else {
		profile.SOCKS5.启用 = null;
		profile.SOCKS5.全局 = false;
		profile.SOCKS5.账号 = '';
	}
...
function 应用当前反代配置(配置 = {}) {
	const _p = 特征码字典[0];
	const 反代 = 配置.反代 || {};
	const profile = 反代.配置集?.[反代.当前配置];
	if (!profile) return;
	反代[_p] = profile[_p] || 'auto';
	反代.SOCKS5 = 复制SOCKS5配置(profile.SOCKS5 || {});
}

function 初始化反代配置集(配置 = {}) {
	const _p = 特征码字典[0];
	if (!配置.反代) 配置.反代 = {};
	if (!配置.反代.SOCKS5) 配置.反代.SOCKS5 = { 启用: null, 全局: false, 账号: '', 白名单: SOCKS5白名单 };
	if (!配置.反代[_p]) 配置.反代[_p] = 'auto';

	const 原配置集 = 配置.反代.配置集 && typeof 配置.反代.配置集 === 'object' ? 配置.反代.配置集 : null;
	const 配置集 = {};
	if (原配置集) {
		for (const [id, profile] of Object.entries(原配置集)) {
			if (反代配置ID合法(id)) 配置集[id] = 规范化反代配置项(profile, 当前反代配置快照(配置.反代));
		}
	}
	if (Object.keys(配置集).length === 0) 配置集.default = 当前反代配置快照(配置.反代);

	let 当前配置 = String(配置.反代.当前配置 || '').trim();
	if (!配置集[当前配置]) 当前配置 = 配置集.default ? 'default' : Object.keys(配置集)[0];
	配置.反代.当前配置 = 当前配置;
	配置.反代.配置集 = 配置集;
	应用当前反代配置(配置);
	return 配置.反代;
...
function 同步当前反代到配置集(配置 = {}) {
	const 当前配置 = 配置.反代?.当前配置;
	if (!当前配置 || !配置.反代?.配置集?.[当前配置]) return;
	配置.反代.配置集[当前配置] = 当前反代配置快照(配置.反代);
}

function 构建反代配置集响应(配置 = {}) {
	初始化反代配置集(配置);
	return {
		success: true,
		active: 配置.反代.当前配置,
		profiles: 配置.反代.配置集,
		current: 当前反代配置快照(配置.反代),
	};
}

async function 处理反代配置集操作(env, 配置, payload = {}) {
	初始化反代配置集(配置);
	const action = String(payload.action || payload.操作 || 'save').toLowerCase();
	const id = 获取反代配置ID(payload);

	if (action === 'switch' || action === 'active') {
		if (!反代配置ID合法(id) || !配置.反代.配置集[id]) throw new Error('反代配置不存在');
		配置.反代.当前配置 = id;
	} else if (action === 'delete') {
		if (!反代配置ID合法(id) || !配置.反代.配置集[id]) throw new Error('反代配置不存在');
		if (Object.keys(配置.反代.配置集).length <= 1) throw new Error('至少需要保留一套反代配置');
		delete 配置.反代.配置集[id];
		if (配置.反代.当前配置 === id) 配置.反代.当前配置 = Object.keys(配置.反代.配置集)[0];
	} else {
		if (!反代配置ID合法(id)) throw new Error('配置ID只能包含字母、数字、下划线、点和短横线，长度 1-64');
		const profile = payload.profile || payload.配置 || payload;
		配置.反代.配置集[id] = 规范化反代配置项(profile, 配置.反代.配置集[id] || 当前反代配置快照(配置.反代));
		if (payload.active === true || payload.启用 === true || action === 'save-active') 配置.反代.当前配置 = id;
	}
...
<title>反代配置集</title>
<style>
:root{color-scheme:light dark;--bg:#f7f8fa;--fg:#17181a;--muted:#657080;--line:#d9dee7;--panel:#fff;--accent:#0f766e;--danger:#b42318}
@media (prefers-color-scheme:dark){:root{--bg:#111418;--fg:#eef1f5;--muted:#9aa4b2;--line:#303742;--panel:#171b21;--accent:#2dd4bf;--danger:#ff8a7a}}
...
main{max-width:980px;margin:0 auto;padding:24px}header{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:18px}
h1{font-size:22px;margin:0}a{color:var(--accent);text-decoration:none}.grid{display:grid;grid-template-columns:260px 1fr;gap:16px}
.panel{background:var(--panel);border:1px solid var(--line);border-radius:8px}.list{padding:8px}.item{width:100%;display:flex;align-items:center;justify-content:space-between;gap:8px;border:0;background:transparent;color:var(--fg);padding:10px;border-radius:6px;text-align:left;cursor:pointer}
.item.active{background:color-mix(in srgb,var(--accent) 16%,transparent)}.item span:last-child{color:var(--muted);font-size:12px}
form{padding:16px;display:grid;gap:12px}.row{display:grid;grid-template-columns:140px 1fr;gap:10px;align-items:center}
label{color:var(--muted)}input,select,textarea{width:100%;border:1px solid var(--line);border-radius:6px;background:transparent;color:var(--fg);padding:9px;font:inherit}
textarea{min-height:72px;resize:vertical}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:4px}
button{border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--fg);padding:9px 12px;font:inherit;cursor:pointer}
button.primary{background:var(--accent);border-color:var(--accent);color:#fff}button.danger{border-color:var(--danger);color:var(--danger)}
.status{min-height:22px;color:var(--muted)}.hint{color:var(--muted);font-size:12px;margin:0}
@media (max-width:720px){main{padding:16px}.grid{grid-template-columns:1fr}.row{grid-template-columns:1fr}}
</style>
</head>
<body>
<main>
<header><h1>反代配置集</h1><a href="/admin">返回后台</a></header>
<div class="grid">
<section class="panel"><div id="profileList" class="list"></div></section>
<section class="panel">
<form id="profileForm">
<div class="row"><label for="id">配置 ID</label><input id="id" name="id" required pattern="[A-Za-z0-9_.-]{1,64}" placeholder="default"></div>
<div class="row"><label for="name">显示名称</label><input id="name" name="name" required placeholder="默认反代"></div>
<div class="row"><label for="type">类型</label><select id="type" name="type"><option value="proxyip">PROXYIP</option><option value="socks5">SOCKS5</option><option value="http">HTTP</option><option value="https">HTTPS</option><option value="turn">TURN</option><option value="sstp">SSTP</option></select></div>
<div class="row"><label for="proxyip">PROXYIP</label><input id="proxyip" name="proxyip" placeholder="proxyip.example.com:443 或 auto"></div>
<div class="row"><label for="account">代理账号</label><input id="account" name="account" placeholder="user:pass@host:port"></div>
<div class="row"><label for="global">全局代理</label><select id="global" name="global"><option value="false">否，按白名单</option><option value="true">是</option></select></div>
<div class="row"><label for="whitelist">白名单</label><textarea id="whitelist" name="whitelist" placeholder="每行一个域名，支持 * 通配符"></textarea></div>
<p class="hint">PROXYIP 类型使用 PROXYIP 字段；SOCKS5/HTTP/HTTPS/TURN/SSTP 类型使用代理账号字段。</p>
<div class="actions"><button class="primary" type="submit">保存</button><button type="button" id="saveActive">保存并启用</button><button type="button" id="activate">启用</button><button type="button" id="create">新建</button><button type="button" id="remove" class="danger">删除</button></div>
...
<script>
var state = { active: "", profiles: {}, selected: "" };
var proxyKey = "PROXYIP";
var $ = function(id){ return document.getElementById(id); };
function setStatus(text){ $("status").textContent = text || ""; }
function profileType(profile){
  var s = profile && profile.SOCKS5 ? profile.SOCKS5 : {};
  return s.启用 || "proxyip";
}
function render(){
  ...
  Object.keys(state.profiles).forEach(function(id){
    ...
    if (!proxyKey) {
      Object.keys(p).forEach(function(k){ if (k !== "名称" && k !== "SOCKS5") proxyKey = k; });
    }
    var btn = document.createElement("button");
    btn.type = "button";
    btn.className = "item" + (id === state.selected ? " active" : "");
    btn.innerHTML = "<span>" + (p.名称 || id) + "</span><span>" + (id === state.active ? "生效中" : profileType(p)) + "</span>";
    btn.onclick = function(){ selectProfile(id); };
...
function selectProfile(id){
  ...
  $("id").value = id || "";
  $("name").value = p.名称 || id || "";
  $("type").value = s.启用 || "proxyip";
  $("proxyip").value = p[proxyKey] || p.PROXYIP || p.proxyip || "auto";
  $("account").value = s.账号 || "";
  $("global").value = s.全局 ? "true" : "false";
  $("whitelist").value = Array.isArray(s.白名单) ? s.白名单.join("\\n") : "";
  render();
}
function formPayload(action){
  ...
  return {
    ...
    profile: {
      ...
      type: type,
      proxyip: $("proxyip").value.trim() || "auto",
      account: $("account").value.trim(),
      global: $("global").value === "true",
      SOCKS5: {
        启用: type === "proxyip" ? null : type,
        全局: $("global").value === "true",
        账号: $("account").value.trim(),
        白名单: $("whitelist").value.split(/\\n|,/).map(function(x){ return x.trim(); }).filter(Boolean)
      }
    }
  };
}
async function api(payload){
  setStatus("保存中...");
  var res = await fetch("/admin/proxy-profiles.json", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
  var data = await res.json();
  if (!res.ok || data.success === false) throw new Error(data.error || "请求失败");
  state.active = data.active;
  state.profiles = data.profiles || {};
  state.selected = payload.id || state.active;
  render();
...
async function load(){
  ...
  state.active = data.active;
  state.profiles = data.profiles || {};
  state.selected = state.active || Object.keys(state.profiles)[0] || "";
  render();
  selectProfile(state.selected);
}
$("profileForm").onsubmit = function(e){ e.preventDefault(); api(formPayload("save")).catch(function(err){ setStatus(err.message); }); };
$("saveActive").onclick = function(){ api(formPayload("save-active")).catch(function(err){ setStatus(err.message); }); };
$("activate").onclick = function(){ api({ action: "switch", id: $("id").value.trim() }).catch(function(err){ setStatus(err.message); }); };
$("create").onclick = function(){ state.selected = ""; $("id").value = ""; $("name").value = ""; $("type").value = "proxyip"; $("proxyip").value = "auto"; $("account").value = ""; $("global").value = "false"; $("whitelist").value = ""; render(); };
$("remove").onclick = function(){ if (confirm("删除当前反代配置？")) api({ action: "delete", id: $("id").value.trim() }).catch(function(err){ setStatus(err.message); }); };
load().catch(function(err){ setStatus(err.message); });
</script>
</body>
</html>`;
}
